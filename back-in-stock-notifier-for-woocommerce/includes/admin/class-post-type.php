				...
				case 'product':
					...
					if ( $prod_obj ) {
						...
						if ( $product_id ) {
							...
							$status_map = array(
								'instock' => array( 'label' => __( 'In Stock', 'back-in-stock-notifier-for-woocommerce' ), 'class' => 'cwg_stock_instock' ),
								'outofstock' => array( 'label' => __( 'Out of Stock', 'back-in-stock-notifier-for-woocommerce' ), 'class' => 'cwg_stock_outofstock' ),
								'onbackorder' => array( 'label' => __( 'On Backorder', 'back-in-stock-notifier-for-woocommerce' ), 'class' => 'cwg_stock_backorder' )
							);
							$status_label = isset( $status_map[ $stock_status ]['label'] ) ? $status_map[ $stock_status ]['label'] : '';
							$status_color_class = isset( $status_map[ $stock_status ]['class'] ) ? $status_map[ $stock_status ]['class'] : '';
							echo '<br>';
							echo '<strong>' . esc_html__( 'Current Stock Status:', 'back-in-stock-notifier-for-woocommerce' ) . '</strong> <span class="' . esc_attr( $status_color_class ) . '">' . esc_html( $status_label ) . '</span>';
