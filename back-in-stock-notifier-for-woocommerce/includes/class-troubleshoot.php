...
if (!class_exists('CWG_Instock_Troubleshoot')) {
	...
	class CWG_Instock_Troubleshoot {
		...
		public function add_settings_field() {
			...
			add_settings_field('cwg_instock_stop_email_staging', __('Enable Stop Sending Email from staging site environment', 'back-in-stock-notifier-for-woocommerce'), array($this, 'stop_sending_email_staging'), 'cwginstocknotifier_settings', 'cwginstock_section_troubleshoot');
		...
		public function stop_sending_email_staging() {
			...
			<input type='checkbox' class='stop_sending_email_staging' name='cwginstocksettings[stop_sending_email_staging]' <?php isset($options['stop_sending_email_staging']) ? checked($options['stop_sending_email_staging'], 1) : ''; ?>
				   value="1" />
			<p><i><?php esc_html_e('Enable this option to disable email notifications when sites are accessed on staging domains that mirror the production site. This prevents unnecessary alerts from a testing environment, ensuring the production email flow remains unaffected', 'back-in-stock-notifier-for-woocommerce'); ?></i></p>
			<?php
		}
		...
		public function stop_sending_email_staging_domain() {
			...
			?>
			<textarea class="staging_domains" name="cwginstocksettings[staging_domains]" rows="4" cols="50"><?php echo esc_html($staging_domains); ?></textarea>
			<p><i><?php esc_html_e('Enter your starting domain(s) of the staging environment. This option is comma-separated (e.g., "dev.", "test.", "staging.", "demo", etc.)', 'back-in-stock-notifier-for-woocommerce'); ?></i></p>
			<?php
		}
